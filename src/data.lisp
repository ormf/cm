...
;;; generated by scheme->cltl from data.scm on 04-Aug-2009 14:11:45
...
(defun encode-interval (interval &optional letters)
  (let ((direction 1)
    ...
    (let ((name (symbol-name quality)))
      (setf quality
            (do ((i 0 (+ i 1)) (e (length interval-names)) (f nil))
                ((or f (= i e)) f)
              (if (find-if (lambda (y)
                             (string-equal name (symbol-name y)))
                 ...
                 (cond (pos
                       ...
                       (next-token-position (str lb len)
                         (loop with chr
                               ...
                               do (setf chr (elt str i))
                               until (find-if
                                      (lambda (x) (char-equal chr x))
                                      ops)
                               finally (return i))))
...
(defmethod amplitude ((amp cons) &optional (softest *softest*)
                      (loudest *loudest*) (power *power*))
  (mapcar (lambda (x) (amplitude x softest loudest power)) amp))

(defmethod amplitude (amp &optional (softest *softest*)
                      (loudest *loudest*) (power *power*))
  (do ((tail *logical-amplitudes* (cddr tail)) (ampl nil))
      ((or (null tail) ampl)
       (if (not ampl)
           (error "'~s' is not a logical amplitude." amp)
           (amplitude ampl softest loudest power)))
    (if (or (and (consp (car tail)) (member amp (car tail)))
            (equal (car tail) amp))
        (setf ampl (cadr tail)))))
