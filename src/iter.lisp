...
;;; generated by scheme->cltl from loop.scm on 04-Aug-2009 14:11:45
    ...
    (if stop
        (let ((end (gensym "v")))
          (if (numberp stop)
              ...
              (progn (push (make-binding end nil) bind)
                     (push `(setf ,end ,stop) init)))
          (setf stop (list test var end))))
    (values (make-loop-clause 'operator 'for 'bindings (reverse bind)
             'initially (reverse init) 'stepping (list step)
             'end-tests (if (not stop) '() (list stop)))
            tail)))
  ...
  (let ((head forms)
    ...
    (do ((next nil))
      ...
      (case next
        ...
        ((then) (if init (loop-error ops head "Duplicate 'then'."))
    ...
    (let ((save forms)
      ...
      (if (not (null forms))
          (if (eq (car forms) 'into)
              (progn (if (null (cdr forms))
                         (loop-error ops save
                          "Missing 'into' variable."))
      ...
      (if (not coll)
          (setf new? t)
          (if (not (compatible-accumulation? oper
                    (collector-type coll)))
        ...
        ((append collect nconc)
         (let ((head nil) (tail nil))
           (if (not new?)
               ...
               (progn (if into
                          (push (make-binding into '(list)) bind))
                      (setf tail (gensym "v"))
                      (setf head (gensym "v"))
           ...
           (if (eq oper 'append)
               (progn (push `(rplacd ,tail (append ,expr (list)))
                            loop)
