...
;;; generated by scheme->cltl from patterns.scm on 04-Aug-2009 14:11:46
...
(progn (defclass pattern (container)
         ((flags :initarg :flags :initform 0 :accessor pattern-flags)
          (data :initarg :of :initarg :notes :initarg :keynums
           ...
           :initarg :intervals :initform '() :accessor pattern-data)
...
(defmethod initialize-instance :after ((obj cycle)
                                       &rest
                                       args) args (let
                                                   ((cyc
                                                     (make-cycl)))
    ...
    (when (null (cycl-tail cycl))
      (let ((mode (next-1 (palindrome-elide obj)))
        ...
        (cond ((member mode '(t 2) :test #'eq)
              ...
              ((member mode
                       '(1 :last last :end end :right right)
                       :test
                       #'eq)
               (setf long (- (* long 2) 1))
...
(defun reset-random-range (obj range adj?)
  (let ((data (car (pattern-data obj))))
    (loop with s = (if (integerp range) 0 0.0)
          for item in data
          do (incf s
                   (if adj?
          ...
          (setf next
                (loop for item =
                          (loop with index = (random range state)
                                ...
                                (when
                                 adjust
                                 (random-item-weight-set!
                                  next
                                  (*
                                   (random-item-weight next)
                                   adjust)))
...
(defmethod initialize-instance :after ((obj graph)
                                       &rest
                                       args) args (let
                                                   ((nodes
                                                     (pattern-data
...
(defun parse-rules (rules table)
  (labels ((getnode (id table rule)
             (or (gethash id table)
                 (if (eq id '*)
                     '*
                     (error "No node for id ~s in rule ~s."
                            id
                            rule))))
...
(defmethod canonicalize-pattern-data ((obj join) data parser inits)
  ...
  (let ((subs (not (some #'pattern? data))))
    (if parser
        (let ((coll (list nil)))
          (do ((tail data (cdr tail)) (num 0) (lst coll))
              ((null tail)
               (values (cdr coll)
...
(defmethod initialize-instance :after ((obj copier)
                                       &rest
                                       args) args (let
                                                   ((data
                                                     (pattern-data
...
(defun make-pattern-binding (typ args)
  (if (not typ)
      ...
      (if (not (and (cdr args) (cddr args) (cdddr args)))
          ...
          (if (let ((t12 (second args))) (and t12 (symbolp t12)))
              (if (eq (third args) '=)
                  (let ((var (second args)) (val (fourth args)))
                    (if (numberp val)
                        ...
                        (if (eq typ 'with)
                            (list var val)
                            (list var `(lambda nil ,val)))))
                  (error "Expected '=' but got '~s' instead: ~s"
                         (third args)
                         args))
              (error "Expected variable name (symbol) but got '~s' instead: ~s"
                     (second args)
                     args)))))
  ...
  (cond ((null form)
        ...
        ((consp option)
         (if (member (car form) option)
             t
             (if err?
                 (error "Expected pattern option from ~S but got ~S instead."
                        option
                        form)
                 nil)))
        (t
...
(defun get-pattern-options (ptyp etyp)
  (do ((tail pattern-options (cdr tail)) (opts (list)))
      ((null tail) opts)
    (if (consp (car tail))
...
(defun expand-pattern-macro (form)
  (let ((vars (list))
    ...
    (do ((tail args) (optn nil) (valu nil) (seen (list)))
        ((null tail) nil)
