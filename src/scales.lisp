...
;;; generated by scheme->cltl from scales.scm on 04-Aug-2009 14:11:45
...
(defun rest? (f) (or (eq f 'r) (eq f ':r) (eq f -1) (equal f 0.0)))
...
(defun ascending-mode-order (notes tuning)
  (setf notes
        (if (let ((t5 (car notes))) (and t5 (symbolp t5)))
            (mapcar (lambda (n)
...
(defmethod interval ((ref symbol) &optional ref2)
  (cond ((note-in-scale? ref *chromatic-scale*)
         (let* ((table (scale-table *chromatic-scale*))
           ...
           (setf letters
                 (mod (- (position-if (lambda
                                       (x)
                                       (member (sd-name entry1) x))
                                      names)
                         (position-if (lambda
                                       (x)
                                       (member (sd-name entry2) x))
                                      names))
                      7))
