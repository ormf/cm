...
;;; generated by scheme->cltl from sco.scm on 04-Aug-2009 14:11:46
...
(progn (defclass sco-file (event-file)
...
(defmethod sco-name ((obj i))
  (if (slot-boundp obj 'ins)
      ...
      (let ((n (symbol-name (class-name (class-of obj)))))
        (if (lower-case-p (elt n 0)) n (string-downcase n)))))
  ...
  (let ((i (gensym)))
    `(let ((,i (floor (car ,pars))))
       (unless (assoc ,i ,d)
         (push (list ,i
                     (length ,pars)
                     (intern (string-upcase (format nil "i~a" ,i))
                             :cm))
               ,d)))))
     ...
     (do ()
       ...
       (if next
           ...
           (let ((raw (file-line in)))
             (if (file-eof? raw)
                 (setf line raw)
                 (let ((pos
                        (do ((i 0 (+ i 1)) (e (length raw)) (f nil))
                            ((or f (= i e)) f)
                          (if (char= (elt raw i) #\;) (setf f i)))))
       ...
       (cond ((file-eof? line) (setf stop t))
             ...
             (t
              (if statement
                  (setf statement
                        (concatenate 'string statement " " line))
                  (progn (format t
...
(defun sco-write-and-load (&rest args) args (car (last args)))
